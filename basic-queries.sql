...
--    Order them in descending order by price. If two or more have the same price, list those in ascending order by product name. 
...
-- Products table identify the type (category) of each item sold by CategoryID.
 select *
 from products
 where CategoryID = 8; 
...
-- from the Products table can tell supplier that supplies each product by SupplierID. 
